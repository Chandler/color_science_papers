...
def sample_unit_sphere(npoints=100):
...
def sphere_coords(fineness=1):
	# step_angle = np.deg2rad(45) / math.pow(2, fineness))

	max_theta = np.deg2rad(360)
	max_phi = np.deg2rad(180)
	thetas = np.linspace(0, max_theta, max_theta/np.deg2rad(45.0))
	phis = np.linspace(0, max_phi, max_phi/np.deg2rad(45.0))
...
def compute_metamere_mismatch_body(
	...
	thetas, phis = sphere_coords()
	for theta in thetas:
		for phi in phis:
			import math
			ax = math.cos(theta) * math.sin(phi)
			ay = math.sin(theta) * math.sin(phi)
			az = math.cos(phi)

			# A is a direction vector and F is its covector
			# a functional R3 -> R
			# (basically just the row vector of D
			direction_vector = [ax,ay,az]
